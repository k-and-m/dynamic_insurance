...
template<class T> class Matrix<T,2> : public Matrix_base<T> {
    const Index d1; //number of rows
    const Index d2; //number of columns
...
public:
    ...
    Index dim1() const { return d1; }    // number of rows
    Index dim2() const { return d2; }    // number of columns
    Index nrows() const { return dim1();}
    Index ncols() const { return dim2();}
...
public:
    ...
    // Column<T,2> column(Index n); // not (yet) implemented: requires strides and operations on columns
...
public:
	...
	Matrix(Index n1 = AGG_SHOCK_SIZE, Index n2 = CAP_SHOCK_SIZE,
		Index n3 = CAP_SHOCK_SIZE, Index n4 = WAGE_SHOCK_SIZE) : Matrix_base<T>(n1*n2*n3*n4), d1(n1), d2(n2), d3(n3), d4(n4) { }

    Matrix(Row<T,4>& a) : Matrix_base<T>(a.dim1()*a.dim2()*a.dim3()*a.dim4(),a.p), d1(a.dim1()), d2(a.dim2()), d3(a.dim3()), d4(a.dim4())
    {
        // std::cerr << "construct 4D Matrix from Row\n";
    }
    ...
    Matrix(const T (&a)[n1][n2][n3][n4]) : Matrix_base<T>(n1*n2*n3), d1(n1), d2(n2), d3(n3), d4(n4)
        // deduce "n1", "n2", "n3" (and "T"), Matrix_base allocates T[n1*n2*n3]
    {
        // std::cerr << "matrix ctor\n";
        for (Index i = 0; i<n1; ++i)
            for (Index j = 0; j<n2; ++j)
                for (Index k = 0; k<n3; ++k)
                	for (Index l = 0; l<n4; ++l)
                		this->elem[i*n2*n3*n4+j*n3*n4+k*n4+l]=a[i][j][k][l];
    }
    ...
    {
        // std::cerr << "copy assignment (" << this->size() << ',' << a.size()<< ")\n";
        if (d1!=a.d1 || d2!=a.d2 || d3!=a.d3 || d4!=a.d4) error("length error in 4D =");
        this->base_assign(a);
        return *this;
    }
    ...
    Index dim2() const { return d2; }    // number of elements in a column
    Index dim3() const { return d3; }    // number of elements in a depth
    Index dim4() const { return d4; }    // number of elements in a spacetime
    ...
    const Row<T,3> operator[](Index n) const { return row(n); }
          ...
          Row<T,3> row(Index n)       { range_check(n,0,0,0); return Row<T,3>(d2,d3,d4,&this->elem[n*d2*d3*d4]); }
    const Row<T,3> row(Index n) const { range_check(n,0,0,0); return Row<T,3>(d2,d3,d4,&this->elem[n*d2*d3*d4]); }

    Row<T,4> slice(Index n)
        // rows [n:d1)
    {
        if (n<0) n=0;
        else if(d1<n) n=d1;    // one beyond the end
        return Row<T,4>(d1-n,d2,d3,d4,this->elem+n*d2*d3*d4);
    }

    const Row<T,4> slice(Index n) const
        // rows [n:d1)
    {
        if (n<0) n=0;
        else if(d1<n) n=d1;    // one beyond the end
    ...
    Row<T,4> slice(Index n, Index m)
        // the rows [n:m)
    {
        if (n<0) n=0;
        if(d1<m) m=d1;    // one beyond the end
        return Row<T,4>(m-n,d2,d3,d4,this->elem+n*d2*d3*d4);

    }

    const Row<T,4> slice(Index n, Index m) const
        // the rows [n:sz)
    {
        if (n<0) n=0;
        if(d1<m) m=d1;    // one beyond the end
        ...
        // if you want pairwise swap, just write it yourself
    {
        ...
        Matrix<T,3> temp = (*this)[i];
        (*this)[i] = (*this)[j];
        (*this)[j] = temp;
    }
};

template<class T> class Matrix<T, 5> : public Matrix_base<T>{
	const Index d1;
	const Index d2;
	const Index d3;
	const Index d4;
...
protected:
	// for use by Row:
	Matrix(Index n1, Index n2, Index n3, Index n4, Index n5, T* p) : Matrix_base<T>(n1*n2*n3*n4*n5, p), d1(n1), d2(n2), d3(n3), d4(n4), d5(n5)
	{
		// std::cerr << "construct 5D Matrix from data\n";
	}

public:

	Matrix(Index n1 = PHI_STATES, Index n2 = AGG_SHOCK_SIZE, Index n3 = CAP_SHOCK_SIZE,
		Index n4 = CAP_SHOCK_SIZE, Index n5 = WAGE_SHOCK_SIZE) : Matrix_base<T>(n1*n2*n3*n4*n5), d1(n1), d2(n2), d3(n3), d4(n4), d5(n5) { }

	Matrix(Row<T, 5>& a) : Matrix_base<T>(a.dim1()*a.dim2()*a.dim3()*a.dim4()*a.dim5(), a.p), d1(a.dim1()), d2(a.dim2()), d3(a.dim3()), d4(a.dim4()), d5(a.dim5())
	{
		// std::cerr << "construct 5D Matrix from Row\n";
	}

	// copy constructor: let the base do the copy:
	Matrix(const Matrix& a) : Matrix_base<T>(a.size(), 0), d1(a.d1), d2(a.d2), d3(a.d3), d4(a.d4), d5(a.d5)
	{
		// std::cerr << "copy ctor\n";
		this->base_copy(a);
	}
	...
	Matrix(const T(&a)[n1][n2][n3][n4][n5]) : Matrix_base<T>(n1*n2*n3*n4), d1(n1), d2(n2), d3(n3), d4(n4), d5(n5)
		// deduce "n1", "n2", "n3" (and "T"), Matrix_base allocates T[n1*n2*n3]
	{
		// std::cerr << "matrix ctor\n";
		for (Index i = 0; i<n1; ++i)
			for (Index j = 0; j<n2; ++j)
				for (Index k = 0; k<n3; ++k)
					for (Index l = 0; l<n4; ++l)
						for (Index m = 0; m<n5; ++m)
							this->elem[i*n2*n3*n4*n5 + j*n3*n4*n5 + k*n4*n5 + l*n4 + m] = a[i][j][k][l][m];
	}

	template<class F> Matrix(const Matrix& a, F f) : Matrix_base<T>(a.size()), d1(a.d1), d2(a.d2), d3(a.d3), d4(a.d4), d5(a.d5)
		// construct a new Matrix with element's that are functions of a's elements:
		// does not modify a unless f has been specifically programmed to modify its argument
		// T f(const T&) would be a typical type for f
	{
	...
	template<class F, class Arg> Matrix(const Matrix& a, F f, const Arg& t1) : Matrix_base<T>(a.size()), d1(a.d1), d2(a.d2), d3(a.d3), d4(a.d4), d5(a.d5)
		// construct a new Matrix with element's that are functions of a's elements:
		// does not modify a unless f has been specifically programmed to modify its argument
		// T f(const T&, const Arg&) would be a typical type for f
	{
	...
	{
		// std::cerr << "copy assignment (" << this->size() << ',' << a.size()<< ")\n";
		if (d1 != a.d1 || d2 != a.d2 || d3 != a.d3 || d4 != a.d4 || d5 != a.d5) error("length error in 5D =");
		this->base_assign(a);
		return *this;
	}
	...
	Index dim3() const { return d3; }    // number of elements in a depth
	Index dim4() const { return d4; }    // number of elements in a spacetime
	Index dim5() const { return d5; }    // number of elements in Aquarius

	Matrix xfer()    // make an Matrix to move elements out of a scope
	{
		Matrix x(dim1(), dim2(), dim3(), dim4(), dim5(), this->data()); // make a descriptor
		this->base_xfer(x);            // transfer (temporary) ownership to x
		return x;
	}

	void range_check(Index n1, Index n2, Index n3, Index n4, Index n5) const
	{
		// std::cerr << "range check: (" << d1 << "," << d2 << "): " << n1 << " " << n2 << "\n";
		if (n1<0 || d1 <= n1) error("5D range error: dimension 1");
		if (n2<0 || d2 <= n2) error("5D range error: dimension 2");
		if (n3<0 || d3 <= n3) error("5D range error: dimension 3");
		if (n4<0 || d4 <= n4) error("5D range error: dimension 4");
		if (n5<0 || d5 <= n5) error("5D range error: dimension 5");
	}

	// subscripting:
	T& operator()(Index n1, Index n2, Index n3, Index n4, Index n5)       { range_check(n1, n2, n3, n4, n5); return this->elem[d2*d3*d4*d5*n1 + d3*d4*d5*n2 + d4*d5*n3 + d5*n4 + n5]; };
	const T& operator()(Index n1, Index n2, Index n3, Index n4, Index n5) const { range_check(n1, n2, n3, n4, n5); return this->elem[d2*d3*d4*d5*n1 + d3*d4*d5*n2 + d4*d5*n3 + d5*n4 + n5]; };

	// slicing (return a row):
	Row<T, 4> operator[](Index n)       { return row(n); }
	const Row<T, 4> operator[](Index n) const { return row(n); }

	Row<T, 4> row(Index n)       { range_check(n, 0, 0, 0, 0); return Row<T, 4>(d2, d3, d4, d5, &this->elem[n*d2*d3*d4*d5]); }
	const Row<T, 4> row(Index n) const { range_check(n, 0, 0, 0, 0); return Row<T, 4>(d2, d3, d4, d5, &this->elem[n*d2*d3*d4*d5]); }

	Row<T, 5> slice(Index n)
		// rows [n:d1)
	{
		if (n<0) n = 0;
		else if (d1<n) n = d1;    // one beyond the end
		return Row<T, 5>(d1 - n, d2, d3, d4, d5, this->elem + n*d2*d3*d4*d5);
	}

	const Row<T, 5> slice(Index n) const
		// rows [n:d1)
	{
		if (n<0) n = 0;
		else if (d1<n) n = d1;    // one beyond the end
	...
	Row<T, 5> slice(Index n, Index m)
		// the rows [n:m)
	{
		if (n<0) n = 0;
		if (d1<m) m = d1;    // one beyond the end
		return Row<T, 5>(m - n, d2, d3, d4, d5, this->elem + n*d2*d3*d4*d5);

	}

	const Row<T, 5> slice(Index n, Index m) const
		// the rows [n:sz)
	{
		if (n<0) n = 0;
		if (d1<m) m = d1;    // one beyond the end
...
template<class T> class Row<T,4> : public Matrix<T,4> {
public:
    Row(Index n1, Index n2, Index n3, Index n4, T* p) : Matrix<T,4>(n1,n2,n3,n4,p)
    {
    }

    Matrix<T,4>& operator=(const T& c) { this->base_apply(Assign<T>(),c); return *this; }

    Matrix<T,4>& operator=(const Matrix<T,4>& a)
    {
        return *static_cast<Matrix<T,4>*>(this)=a;
    }
};

//-----------------------------------------------------------------------------
template<class T> class Row<T, 5> : public Matrix<T, 5>{
public:
	Row(Index n1, Index n2, Index n3, Index n4, Index n5, T* p) : Matrix<T, 5>(n1, n2, n3, n4, n5, p)
	{
	}

	Matrix<T, 5>& operator=(const T& c) { this->base_apply(Assign<T>(), c); return *this; }

	Matrix<T, 5>& operator=(const Matrix<T, 5>& a)
	{
		return *static_cast<Matrix<T, 5>*>(this) = a;
	}
};
